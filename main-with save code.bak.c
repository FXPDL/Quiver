...
double modulation_step_size = 30;

volatile int flash_saving = 0;
volatile int saving_state = 0;
volatile int top_push_state = 6;
...
int temp_tap_count = 0;
long temp_tap_time = 0;

volatile long tap_longpress_timer = 0;
int tap_longpress_time = 5; //in seconds
...
#define SPI_Confirm LATBbits.LATB1
...
signed int mod1[] = {0, 21, 40, 59, 74, 86, 95, 99, 100, 95, 87, 75, 60, 42, 22, 2, -19, -39, -57, -73, -85, -94, -99, -100, -95, -88, -76, -61, -43, -24}; //sin wave
...
long mod1_amplitude = 200;
int modulation_frequency = 1;
...
void main(void) {
    ...
    while (1) {
        ...
        if (SPI_Aval == 1) {
            ...
            LED_bypass = 0;
            wait_ms(2);
            for (count = 0; count < preset; count++) {
                LED_bypass = 1;
                wait_ms(40);
                LED_bypass = 0;
                wait_ms(40);
        ...
        //  4) freeze program until button released
        //--------------------------------------------------
        if (switch_bypass == 0) { //is switch pressed?
            debounce_bypass++; //increment debounce timer
            if (debounce_bypass > 25) { //if switch has been on for 25 cycles, switch is confirmed on.
            ...
            debounce_bypass = 0; // if switch hasn't been on 
        }
            ...
            if (debounce_count > 25) {
                ...
                if (tap_timer >= 1563) {
                    ...
                    while (switch_tap == 0) {
                        ...
                        if (tap_longpress_timer > 1000 && saving_state == 0) {
                            //initiate savings
                            flash_saving == 0;
                            saving_state = 1; 

                            //put saving code here, set saving_state = 0 when it is done.
                    ...
                    if (temp_tap_count == 1) {
                        LED_RJ45_Yellow = 1;
                        //Don't overwrite the existing tap settings until at least 2 clicks.
                        //First click, save the values into temp settings.
                        temp_tap_time = tap_timer;
                    } else {
                        if (temp_tap_count == 2) {
                            //Move the temp value into slot 1 and keep on trucking!
                            tap_history[1] = tap_timer;

                            saving_state = 0;  //This should be cleared when saving is done!
                        }
                        if (tap_iteration <= 4) {
        ...
        if (mod_timer >= mod_delay_time) {
            ...
            int fancy_mod_cycle = baseline_delay_time;
           ...
           /* if (bottom_push_state > 2) {
                if (mod_counter >= 31) {
                    mod_counter = 0;
               }                
            } else {
                if (mod_counter > fancy_mod_cycle) {
                    mod_counter = 0;
                }
            }


            modulation_step_size = 360 / fancy_mod_cycle; 
            */
            ...
            switch (bottom_push_state) {
                case 1:
                   ...
                   // mod_value = round(125 * sin(modulation_frequency * modulation_step_size * mod_counter * rad_conversion)) ; //mod1[mod_counter]; //
                    chorus = 0;
                    break;
                case 2:
                    mod_value = mod2[mod_counter];
                    //mod_value = round(150 * sin(modulation_frequency * modulation_step_size * mod_counter * rad_conversion)) ; //mod1[mod_counter]; //
                    chorus = 0;
                    break;
                case 3:
